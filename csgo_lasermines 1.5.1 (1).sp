...
new Handle:h_enable, bool:b_enable,#pragma semicolon 1
...
public OnPluginStart()
{
	...
	CreateConVar("csgo_lasermines_version", PLUGIN_VERSION, PLUGIN_NAME, FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);

	new Handle:CVar;
	HookConVarChange((CVar = CreateConVar("sm_lasermines_enable", "1", "0 - disabled, 1 - enabled for all teams, 2 - only for Terrorists, 3 - only for Counter-terrorists", FCVAR_NOTIFY, true, _, true, 3.0)), CVarChanged_Enable);
	i_enable = GetConVarInt(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_welcome_message", "0", "Show Plugin Message at the beginning of each round.", _, true, _, true, 1.0)), CVarChanged_Msg);
	b_message = GetConVarBool(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_amount", "3", "The amount to give laser mines to a player each spawn (if buy mode is disabled, -1 = Infinity)", _, true, -1.0)), CVarChanged_Amt);
	i_amount = GetConVarInt(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_maxamount", "3", "The maximum amount of laser mines a player can carry. (0-Unlimited)", _, true, _, true, 1024.0)), CVarChanged_MaxAmt);
	i_maxamount = GetConVarInt(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_death_purge", "0", "Remove mines after the death of their owner.", _, true, _, true, 1.0)), CVarChanged_Purge);
	b_purge = GetConVarBool(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_damage", "500", "The damage to deal to a player by the laser", _, true, 1.0, true, 100000.0)), CVarChanged_Dmg);
	i_damage = GetConVarInt(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_explode_damage", "80", "The damage to deal to a player when a laser mine breaks", _, true, _, true, 100000.0)), CVarChanged_Explode);
	i_explode_damage = GetConVarInt(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_explode_radius", "250", "The radius of the explosion", _, true, 1.0, true, 100000.0)), CVarChanged_Radius);
	i_explode_radius = GetConVarInt(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_health", "150", "The laser mines health. 0 = never breaked", _, true, _, true, 100000.0)), CVarChanged_HP);
	i_health = GetConVarInt(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_activatetime", "2", "The delay of laser mines' activation", _, true, _, true, 10.0)), CVarChanged_Time);
	f_activate_time = GetConVarFloat(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_buymode", "0", "Enables buy mode. In this mode you will have to buy mines", FCVAR_NOTIFY, true, _, true, 1.0)), CVarChanged_Mode);
	b_use_buy_mode = GetConVarBool(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_buyzone", "1", "Whether a player have to stay in buy zone to buy mines", FCVAR_NOTIFY, true, _, true, 1.0)), CVarChanged_Zone);
	b_should_buy_zone = GetConVarBool(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_price", "0", "The price of the laser mines", FCVAR_NOTIFY, true)), CVarChanged_Price);
	...
	GetConVarString(CVar, s_color_t, sizeof(s_color_t));
	...
	GetConVarString(CVar, s_color_ct, sizeof(s_color_ct));

	HookConVarChange((CVar = CreateConVar("sm_lasermines_allow_pickup", "1", "Allow players to pickup their planted lasermines", _, true, 0.0, true, 1.0)), CVarChanged_PickUp);
	b_pickup = GetConVarBool(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_allow_friendly_pickup", "0", "Allow players to pickup allies planted lasermines", _, true, 0.0, true, 1.0)), CVarChanged_FPickUp);
	b_friendly_pickup = GetConVarBool(CVar);

	HookConVarChange((CVar = CreateConVar("sm_lasermines_allow_enemy_pickup", "0", "Allow players to pickup enemys planted lasermines", _, true, 0.0, true, 1.0)), CVarChanged_EPickUp);
	...
	GetConVarString(CVar, s_wpn_dmg, sizeof(s_wpn_dmg));

	HookConVarChange((CVar = CreateConVar("sm_lasermines_hs", "1", "Headshot On/Off", _, true, 0.0, true, 1.0)), CVarChanged_HS);
...
public CVarChanged_Enable(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	i_enable = GetConVarInt(CVar);
}

public CVarChanged_Msg(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	b_message = GetConVarBool(CVar);
}

public CVarChanged_Amt(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	i_amount = GetConVarInt(CVar);
	LookupClients();
}

public CVarChanged_MaxAmt(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	i_maxamount = GetConVarInt(CVar);
	LookupClients();
}

public CVarChanged_Purge(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	b_purge = GetConVarBool(CVar);
}

public CVarChanged_Dmg(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	i_damage = GetConVarInt(CVar);
}

public CVarChanged_Explode(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	i_explode_damage = GetConVarInt(CVar);
}

public CVarChanged_Radius(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	i_explode_radius = GetConVarInt(CVar);
}

public CVarChanged_HP(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	i_health = GetConVarInt(CVar);
}
...
public CVarChanged_Mode(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	b_use_buy_mode = GetConVarBool(CVar);
}

public CVarChanged_Zone(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	b_should_buy_zone = GetConVarBool(CVar);
}

public CVarChanged_Price(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	i_price = GetConVarInt(CVar);
}

public CVarChanged_TColor(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	strcopy(s_color_t, sizeof(s_color_t), newValue);
}

public CVarChanged_CtColor(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	strcopy(s_color_ct, sizeof(s_color_ct), newValue);
}

public CVarChanged_PickUp(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	b_pickup = GetConVarBool(CVar);
}

public CVarChanged_FPickUp(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	b_friendly_pickup = GetConVarBool(CVar);
}

public CVarChanged_EPickUp(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	b_enemy_pickup = GetConVarBool(CVar);
}

public CVarChanged_Wpn(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	strcopy(s_wpn_dmg, sizeof(s_wpn_dmg), newValue);
}

public CVarChanged_HS(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	b_lm_hs = GetConVarBool(CVar);
}

public CVarChanged_FF(Handle:CVar, const String:oldValue[], const String:newValue[])
{
	b_friendlyfire = GetConVarBool(CVar);
}
...
LookupClients()
{
	for(new i = 1; i <= MaxClients; i++) OnClientConnected(i);
}
...
public OnClientDisconnect(client)
{
	...
	new i = -1;
	while ((i = FindEntityByClassname(i, "prop_physics_override")) != -1 && GetClientByLasermine(i) == client)
...
public OnTouchedByEntity(const String:output[], caller, activator, Float:delay)
{
	...
	new dummy_caller = caller, dummy_owner = owner, damage = i_damage, dummy_lasermine = lasermine;
...
public Action:Cmd_BuyMines(client, argc)
{
	// client = 0	or	disabled	or	buy mode disabled	or	unlimited amount	or	client is not in game
	if(!client || !i_enable || !b_use_buy_mode || i_clients_myamount[client] == -1 || !IsClientInGame(client))
	...
	if(i_enable > 1 && team != i_enable)
	{
		PrintHintText(client, "%t", i_enable == 2 ? "Only for Terrorists" : "Only for Counter-terrorists");
		return Plugin_Handled;	// Stop trigger the command
	}
	// If  buy zone mode is enabled and player is out of buy zone range
	if(b_should_buy_zone && !GetEntData(client, gInBuyZone, 1))
	{
	...
	new dummy_amount = amount, cost = i_price, boughtamount;
	...
	do
	{
		if(i_clients_maxlimit[client] && (i_clients_amount[client] >= i_clients_maxlimit[client]) || (i_buy_limit[client] >= i_clients_maxlimit[client]))
		{
...
public Action:Cmd_PlantMine(client, argc)
{
	if(!client || !i_enable || !IsClientInGame(client))
		return Plugin_Continue;
	...
	switch(team)
	{
		case 2:	StringToColor(s_color_t, color, 255);
		case 3:	StringToColor(s_color_ct, color, 255);
		default :
		{
			PrintHintText(client, "%t", "Can't use, while spec");
			return Plugin_Handled;
	...
	new Float:delay_time = f_activate_time, dummy_damage = i_explode_damage, dummy_radius = i_explode_radius, health = i_health, dummy_color[3];
	dummy_color = color;
	...
	{
		case -1:	PrintHintText(client, "%t", "Infinity mines");
		default :
		{
			i_clients_amount[client]--;
			PrintHintText(client, "%t", "Mines", i_clients_amount[client]);
...
public Action:Cmd_RemoveMines(client, argc)
{
	if(!b_pickup || IsFakeClient(client) || !IsPlayerAlive(client))
		return Plugin_Handled;

	new i = -1;
	while ((i = FindEntityByClassname(i, "prop_physics_override")) != -1 && GetClientByLasermine(i) == client)
		PickupLasermine(client, i, client);
...
OnButtonPressed(client)
{
	...
	if(TR_DidHit(trace) && (ent = TR_GetEntityIndex(trace)) > MaxClients)
	{
		...
		if((b_friendly_pickup && GetClientTeam(owner) == GetClientTeam(client)) || b_enemy_pickup)
			PickupLasermine(client, ent, owner);
		else
		{
			EmitSoundToAll(SND_CANTBUY, ent);
			decl String:nick[MAX_NAME_LENGTH];
			if(GetClientName(owner, nick, sizeof(nick)))
...
PickupLasermine(client, lasermine, owner)
{
	...
	if(i_clients_amount[client] >= 0)
		PrintHintText(client, "%t", "Mines", i_clients_amount[client]);
	else PrintHintText(client, "%t", "Infinity mines");
	EmitSoundToClient(client, SND_BUYMINE);
...
stock PlantMine(client, Float:activation_delay = 0.0, explode_damage, explode_radius, const health = 0, const color[3] = {255, 255, 255})
{
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype)
{
	if(IsEntityLasermine(victim))
	{
		...
		{
			static client;
			if((client = GetClientByLasermine(victim)) != -1 && client != attacker && GetClientTeam(client) == GetClientTeam(attacker))
				return Plugin_Handled;
		}
...
public Native_AddMines(Handle:plugin, numParams)
{
	...
	if(limit && i_clients_amount[client] > i_clients_maxlimit[client])
...
public OnTouchedByEntity(const String:output[], caller, activator, Float:delay)
{
	...
	if (owner == -1 || lasermine == -1 || activator == owner || ZP_IsPlayerHuman(activator))
	{
		return;
	}
...
// Create hook of zombieplague , which called when you will buy item
public Action ZP_OnExtraBuyCommand(client, String:sExtraItemCommand[])
{
	// Verify that client alive and connect (zombieplague.inc have this stock and this IsPlayerHasWeapon too)
	if(!IsPlayerExist(client))
		return Plugin_Handled;
	

	// Cheking string, which you will write in "command" in zp_extraitems.txt
	if(ZP_ExtraItemRegister(sExtraItemCommand, "lasermines"))
	{
		// If client have weapon or zombie, stop
		if(ZP_IsPlayerZombie(client))
		{
			// If you will return Plugin_Handled, 
			// ammopacks do not take, and in hintbox client will see "You can not buy it!"
			return Plugin_Handled;
		}
...
public Action:Command_PlantMine(client, argc)
{
	...
	{
		PrintHintText(client, "STETE: %i", ZP_GetRoundState(STATE_MODE_STARTED));
		return Plugin_Handled;
	}
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype)
{
	if (IsEntityLasermine(victim))
	{
		if (1 <= attacker <= MaxClients)
		{
			...
			if ((client != -1) && (client != attacker) && (ZP_IsPlayerHuman(attacker)))
			{
				return Plugin_Handled;
			}
			return Plugin_Continue;
